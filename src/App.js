...
const anyChecked = exercises.some(ex => checked[sessionKey + “*” + ex.id]);
const volume = calcVolume(
exercises,
Object.fromEntries(exercises.map(ex => [ex.id, checked[sessionKey + “*” + ex.id]])),
...
.filter(ex => checked[sessionKey + “*” + ex.id])
.map(ex => {
let w;
if (ex.barbell) {
const perSide = parseFloat((vals[sessionKey + “*” + ex.id] && vals[sessionKey + “*” + ex.id].perSide)) || 0;
w = 45 + perSide * 2;
} else {
w = parseFloat((vals[sessionKey + “*” + ex.id] && vals[sessionKey + “*” + ex.id].weight)) || 0;
}
const s = parseInt((vals[sessionKey + “*” + ex.id] && vals[sessionKey + “*” + ex.id].setsDone) || ex.sets);
const r = parseInt(ex.reps) || 0;
...
const nc = { …checked };
exercises.forEach(ex => delete nc[sessionKey + “*” + ex.id]);
setChecked(nc);
showSave(true);
} else showSave(false);
};
