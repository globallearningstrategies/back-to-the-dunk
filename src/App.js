...
function TabataTimer({ onLog }) {
  ...
  return (
    <div style={{ ...S.card(COLORS.green + "44"), background: COLORS.green + "08" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: phase !== "idle" ? 12 : 0 }}>
        <div>
...
function StatsTab({ history, weightLog }) {
  ...
  const totalMiles = treadmillSessions.reduce((a, h) => {
    const ex = h.exercises?.[0];
    return a + (ex?.miles || 0);
  }, 0);
  const totalTreadmillMin = treadmillSessions.reduce((a, h) => a + (h.exercises?.[0]?.duration || 0), 0);
  const avgSpeed = treadmillSessions.length > 0
    ? (treadmillSessions.reduce((a, h) => a + (h.exercises?.[0]?.speed || 0), 0) / treadmillSessions.filter(h => h.exercises?.[0]?.speed > 0).length || 0).toFixed(1)
    : 0;
  const avgIncline = treadmillSessions.length > 0
    ? (treadmillSessions.reduce((a, h) => a + (h.exercises?.[0]?.incline || 0), 0) / treadmillSessions.filter(h => h.exercises?.[0]?.incline > 0).length || 0).toFixed(1)
    : 0;
  ...
  gymSessions.forEach(h => { if (h.session_name?.includes("DAY A")) sessionCount.A++; else if (h.session_name?.includes("DAY B")) sessionCount.B++; else if (h.session_name?.includes("DAY C")) sessionCount.C++; });
  ...
  const currentWeight = weightLog[0]?.weight || startWeight;
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "flex-end", height: 80 }}>
            {weekKeys.map((k, i) => (
              <div key={k} style={{ flex: 1, textAlign: "center" }}>
                <div style={{ background: COLORS.orange + "33", borderRadius: "4px 4px 0 0", height: Math.max(4, (weekVals[i] / maxWeekVol) * 70), display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
        ...
        {tab === "workout" && (
          <>
            ...
            <div style={{ ...S.card(restDay ? "#333" : COLORS.orange + "44"), background: restDay ? "#111" : COLORS.orange + "11", marginBottom: 14 }}>
              ...
              {restDay
                ? <div style={{ fontSize: 14 }}>Rest today. Last session was {daysAgo(lastGymSession?.logged_at)}. Recovery is training.</div>
            ...
            <div style={S.card(sessionColor + "44")}>
              ...
              <button onClick={logSession} disabled={!anyChecked}
                style={{ ...S.btn(sessionColor), width: "100%", marginTop: 16, fontSize: 14, padding: 12, opacity: anyChecked ? 1 : 0.4, boxShadow: anyChecked ? "0 0 18px " + sessionColor + "66" : "none", transition: "all .3s" }}>
                {anyChecked ? "Log Session" : "Check at least one exercise to log"}
              </button>
            </div>
