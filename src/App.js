...
const S = {
  ...
  tab: (a) => ({ flex: 1, padding: "12px 4px", border: "none", background: "none", fontFamily: "Georgia, serif", fontSize: 13, cursor: "pointer", color: a ? COLORS.orange : COLORS.muted, borderBottom: a ? `2px solid ${COLORS.orange}` : "2px solid transparent" }),
...
export default function App() {
  ...
  const logSession = async () => {
    ...
    const payload = {
      ...
      color: sessionColor,
      total_volume: exVols.reduce((a, e) => a + e.volume, 0),
      exercises: exVols,
    };
  ...
  const lastDay = lastSession ? Math.floor((Date.now() - new Date(lastSession.logged_at)) / 86400000) : null;
  ...
  return (
    <div style={S.app}>
      <div style={S.header}>
        ...
        <div style={{ fontSize: 11, color: saveMsg.startsWith("✓") ? COLORS.green : "#ff4444" }}>{saveMsg}</div>
      </div>
        ...
        {tab === "workout" && (
          <>
            <div style={{ ...S.card(restDay ? "#333" : COLORS.orange + "44"), background: restDay ? "#111" : COLORS.orange + "11", marginBottom: 16 }}>
              ...
              {restDay ? (
                <div style={{ fontSize: 15 }}>🛌 <strong>Rest today</strong> — last session was {daysAgo(lastSession?.logged_at)}. Recovery is training.</div>
              ) : (
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              <button onClick={() => setActiveSession(null)} style={{ ...S.btn(COLORS.orange, activeSession !== null), fontSize: 12, padding: "7px 14px" }}>Day 1</button>
              {SESSIONS.map((s, i) => (
            ...
            <div style={S.card(sessionColor + "44")}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  <div style={{ ...S.badge(sessionColor), marginBottom: 6 }}>{activeSession === null ? "INTRO" : SESSIONS[activeSession].location.toUpperCase()}</div>
              ...
              return (
                <div key={h.id} style={S.card(h.color + "33")}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ ...S.badge(h.color), marginBottom: 4 }}>{daysAgo(h.logged_at)}</div>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      {h.total_volume > 0 && <div style={{ color: h.color, fontWeight: "bold", fontSize: 14 }}>{h.total_volume.toLocaleString()} lbs</div>}
              ...
              {last8.length >= 2 && (
                <div style={S.card()}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: COLORS.muted, marginTop: 4 }}>
                    <span>{new Date(last8[0].logged_at).toLocaleDateString("en-CA", { month: "short", day: "numeric" })}</span>
                    <span>{new Date(last8[last8.length - 1].logged_at).toLocaleDateString("en-CA", { month: "short", day: "numeric" })}</span>
                  </div>
                </div>
              )}
              {weightLog.length === 0 && <div style={{ textAlign: "center", color: COLORS.muted, padding: 30, fontSize: 13 }}>No weigh-ins yet. Log your first one above.</div>}
              {weightLog.map((e, i) => {
          ...
          <>
            ...
            {STACKS.map((s, i) => (
              <div key={i} style={{ ...S.card(), display: "flex", gap: 12, alignItems: "center", padding: 12 }}>
                <div style={{ background: COLORS.orange + "22", color: COLORS.orange, borderRadius: 8, padding: "6px 10px", fontSize: 11, minWidth: 80, textAlign: "center" }}>{s.trigger}</div>
